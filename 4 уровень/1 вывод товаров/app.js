...
function clickHandler(event) {
  ...
  //в showCategory надо передать строку с типом категории, тип берите из атрибута data-type у кнопки, по которой кликнули.
  showCategory(event.target.getAttribute("data-type"));
}

/**
 * Функция берет товары (объекты) из соответствующего массива phones, tablets или tv. Генерирует разметку, используя getProductMarkup и вставляет в .products
 * @param {string} category сюда должно прилетать значение атрибута data-type у кнопки, по которой кликнули.
 */
function showCategory(category) {
  let productsArray = products[category];
  /*for (let i = 0; i < productsArray.length; i++) {
    productsList.innerHTML += getProductMarkup(productsArray[i]);
  }*/
  productsArray.forEach(function (currentCategory) {
    productsList.innerHTML += getProductMarkup(currentCategory);
  });
}
 ...
 * @returns {string} html-разметка для товара по аналогии из комментария в верху этого файла.
 */
